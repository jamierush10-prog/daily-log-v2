...
export default function Home() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addDoc(collection(db, "logs"), {
        ...
        dateString: date, // Helper for filtering
        createdAt: new Date()
      });
      setStatus('Saved!');
      setEntry(''); 
  ...
  const generateReport = async () => {
    ...
    // Get logs specifically for the selected DATE
    ...
    // Format the logs into text
    let logText = dayLogs.map(l => `[${l.timestamp.split('T')[1]}] ${l.type}: ${l.entry}`).join('\n');

    // Create the AI Prompt
    const prompt = `Act as my Executive Officer. Here are my logs for ${date}. 
Based on these, generate:
1. An After Action Report (AAR) summarizing what was accomplished.
2. A Plan of the Day (POD) for tomorrow, prioritizing any "Do" items not marked "Done".
